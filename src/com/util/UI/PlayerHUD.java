...
import com.main.components.Component;
import com.main.components.ComponentObserver;
import com.main.components.Entity;
import com.main.components.EntityConfig;
import com.main.components.InventoryItem;
import com.main.components.MapManager;
import com.main.components.Utility;
import com.main.components.EngineGameStart.ScreenType;
import com.main.components.InventoryItem.ItemTypeID;
import com.util.audio.AudioManager;
import com.util.audio.AudioObserver;
import com.util.audio.AudioSubject;
import com.util.battle.BattleObserver;
import com.util.dialog.ConversationGraph;
import com.util.dialog.ConversationGraphObserver;
import com.util.profile.ProfileManager;
import com.util.profile.ProfileObserver;
import com.util.quest.QuestGraph;
import com.util.screens.MainGameScreen;
import com.util.sfx.ClockActor;
import com.util.sfx.ScreenTransitionAction;
import com.util.sfx.ScreenTransitionActor;
import com.util.sfx.ShakeCamera;

public class PlayerHUD implements Screen, AudioSubject, ProfileObserver,
		ComponentObserver, ConversationGraphObserver, StoreInventoryObserver,
		BattleObserver, InventoryObserver, StatusObserver {
	private static final String TAG = PlayerHUD.class.getSimpleName();

	private Stage _stage, _stage2;
	private Viewport _viewport;
	private Camera _camera;
	private Entity _player;
	private Texture dpad, aButton, bButton, xButton, yButton, lButton, rButton,
	...
	public PlayerHUD(Camera camera, Entity player, MapManager mapMgr) {
		...
		_stage2 = new Stage(_viewport);
		// _stage.setDebugAll(true);
		dpad = new Texture(Gdx.files.internal("sprites/dPad.png"));

		aButton = new Texture(Gdx.files.internal("sprites/A_button.png"));
		bButton = new Texture(Gdx.files.internal("sprites/B_button.png"));
		xButton = new Texture(Gdx.files.internal("sprites/X_button.png"));
		yButton = new Texture(Gdx.files.internal("sprites/Y_button.png"));
		lButton = new Texture(Gdx.files.internal("sprites/L_button.png"));
		rButton = new Texture(Gdx.files.internal("sprites/R_button.png"));
		startButton = new Texture(Gdx.files.internal("sprites/Start.png"));
		selectButton = new Texture(Gdx.files.internal("sprites/Select.png"));
		...
		_json = new Json();
		_messageBoxUI = new Dialog("Message", Utility.STATUSUI_SKIN,
				"solidbackground") {
			{
				button("OK");
				text(INVENTORY_FULL);
		...
		_messageBoxUI.pack();
		_messageBoxUI.setPosition(
				_stage.getWidth() / 2 - _messageBoxUI.getWidth() / 2,
		...
		TextButton zoomInButton = _statusUI.getZoomInButton();
		zoomInButton.addListener(new ClickListener() {
			...
			public void touchUp(InputEvent event, float x, float y,
					int pointer, int button) {
				_mapMgr.decreasePlayerUnitScaled();
			}
		});

		TextButton zoomOutButton = _statusUI.getZoomOutButton();
		zoomOutButton.addListener(new ClickListener() {
			...
			public void touchUp(InputEvent event, float x, float y,
					int pointer, int button) {
				_mapMgr.increasePlayerUnitScaled();
			}
		});
		...
		// Music/Sound loading
		notify(AudioObserver.AudioCommand.MUSIC_LOAD,
				AudioObserver.AudioTypeEvent.MUSIC_BATTLE);
		notify(AudioObserver.AudioCommand.MUSIC_LOAD,
				AudioObserver.AudioTypeEvent.MUSIC_LEVEL_UP_FANFARE);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_COIN_RUSTLE);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_CREATURE_PAIN);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_PLAYER_PAIN);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_PLAYER_WAND_ATTACK);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_EATING);
		notify(AudioObserver.AudioCommand.SOUND_LOAD,
				AudioObserver.AudioTypeEvent.SOUND_DRINKING);
	}
	...
	public void addTransitionToScreen() {
		...
		_stage.addAction(Actions.sequence(Actions.addAction(
				ScreenTransitionAction.transition(
						ScreenTransitionAction.ScreenTransitionType.FADE_IN, 1),
				_transitionActor)));
	}
	...
	public void onNotify(ProfileManager profileManager, ProfileEvent event) {
		...
		case PROFILE_LOADED:
			...
			if (firstTime) {
				InventoryUI.clearInventoryItems(_inventoryUI
						.getInventorySlotTable());
				InventoryUI.clearInventoryItems(_inventoryUI
						.getEquipSlotTable());
				...
				Array<ItemTypeID> items = _player.getEntityConfig()
						.getInventory();
				Array<InventoryItemLocation> itemLocations = new Array<InventoryItemLocation>();
				for (int i = 0; i < items.size; i++) {
					itemLocations.add(new InventoryItemLocation(i, items.get(i)
							.toString(), 1, InventoryUI.PLAYER_INVENTORY));
				}
				InventoryUI.populateInventory(
						_inventoryUI.getInventorySlotTable(), itemLocations,
						_inventoryUI.getDragAndDrop(),
						InventoryUI.PLAYER_INVENTORY, false);
				profileManager.setProperty("playerInventory", InventoryUI
						.getInventory(_inventoryUI.getInventorySlotTable()));
				...
				profileManager
						.setProperty("currentTime", _clock.getTotalTime());
			} else {
				int goldVal = profileManager.getProperty("currentPlayerGP",
						Integer.class);

				Array<InventoryItemLocation> inventory = profileManager
						.getProperty("playerInventory", Array.class);
				InventoryUI.populateInventory(
						_inventoryUI.getInventorySlotTable(), inventory,
						_inventoryUI.getDragAndDrop(),
						InventoryUI.PLAYER_INVENTORY, false);

				Array<InventoryItemLocation> equipInventory = profileManager
						.getProperty("playerEquipInventory", Array.class);
				if (equipInventory != null && equipInventory.size > 0) {
					_inventoryUI.resetEquipSlots();
					InventoryUI.populateInventory(
							_inventoryUI.getEquipSlotTable(), equipInventory,
							_inventoryUI.getDragAndDrop(),
							InventoryUI.PLAYER_INVENTORY, false);
				}

				Array<QuestGraph> quests = profileManager.getProperty(
						"playerQuests", Array.class);
				_questUI.setQuests(quests);

				int xpMaxVal = profileManager.getProperty("currentPlayerXPMax",
						Integer.class);
				int xpVal = profileManager.getProperty("currentPlayerXP",
						Integer.class);

				int hpMaxVal = profileManager.getProperty("currentPlayerHPMax",
						Integer.class);
				int hpVal = profileManager.getProperty("currentPlayerHP",
						Integer.class);

				int mpMaxVal = profileManager.getProperty("currentPlayerMPMax",
						Integer.class);
				int mpVal = profileManager.getProperty("currentPlayerMP",
						Integer.class);

				int levelVal = profileManager.getProperty("currentPlayerLevel",
						Integer.class);
				...
				float totalTime = profileManager.getProperty("currentTime",
						Float.class);
		...
		case SAVING_PROFILE:
			...
			profileManager.setProperty("currentPlayerGP",
					_statusUI.getGoldValue());
			profileManager.setProperty("currentPlayerLevel",
					_statusUI.getLevelValue());
			profileManager.setProperty("currentPlayerXP",
					_statusUI.getXPValue());
			profileManager.setProperty("currentPlayerXPMax",
					_statusUI.getXPValueMax());
			profileManager.setProperty("currentPlayerHP",
					_statusUI.getHPValue());
			profileManager.setProperty("currentPlayerHPMax",
					_statusUI.getHPValueMax());
			profileManager.setProperty("currentPlayerMP",
					_statusUI.getMPValue());
			profileManager.setProperty("currentPlayerMPMax",
					_statusUI.getMPValueMax());
		...
		case CLEAR_CURRENT_PROFILE:
			...
			profileManager.setProperty("playerInventory",
					new Array<InventoryItemLocation>());
			profileManager.setProperty("playerEquipInventory",
					new Array<InventoryItemLocation>());
	...
	public void onNotify(String value, ComponentEvent event) {
		...
		case LOAD_CONVERSATION:
			...
			if (config.getItemTypeID().equalsIgnoreCase(
					InventoryItem.ItemTypeID.NONE.toString())) {
				EntityConfig configReturnProperty = ProfileManager
						.getInstance().getProperty(config.getEntityID(),
								EntityConfig.class);
		...
		case SHOW_CONVERSATION:
			...
			if (configShow.getEntityID().equalsIgnoreCase(
					_conversationUI.getCurrentEntityID())) {
				_conversationUI.setVisible(true);
			}
			break;
		case HIDE_CONVERSATION:
			EntityConfig configHide = _json.fromJson(EntityConfig.class, value);
			if (configHide.getEntityID().equalsIgnoreCase(
					_conversationUI.getCurrentEntityID())) {
				_conversationUI.setVisible(false);
			}
			break;
		...
		case PLAYER_HAS_MOVED:
			if (_battleUI.isBattleReady()) {
				...
				notify(AudioObserver.AudioCommand.MUSIC_PLAY_LOOP,
						AudioObserver.AudioTypeEvent.MUSIC_BATTLE);
				_battleUI.toBack();
	...
	public void onNotify(ConversationGraph graph, ConversationCommandEvent event) {
		...
		case LOAD_STORE_INVENTORY:
			...
			Array<InventoryItemLocation> inventory = InventoryUI
					.getInventory(_inventoryUI.getInventorySlotTable());
			_storeInventoryUI.loadPlayerInventory(inventory);

			Array<InventoryItem.ItemTypeID> items = selectedEntity
					.getEntityConfig().getInventory();
			Array<InventoryItemLocation> itemLocations = new Array<InventoryItemLocation>();
			for (int i = 0; i < items.size; i++) {
				itemLocations.add(new InventoryItemLocation(i, items.get(i)
						.toString(), 1, InventoryUI.STORE_INVENTORY));
			}
		...
		case EXIT_CONVERSATION:
			...
			break;
		case ACCEPT_QUEST:
			Entity currentlySelectedEntity = _mapMgr
					.getCurrentSelectedMapEntity();
			if (currentlySelectedEntity == null) {
				break;
			}
			EntityConfig config = currentlySelectedEntity.getEntityConfig();

			QuestGraph questGraph = _questUI.loadQuest(config
					.getQuestConfigPath());
			...
			if (questGraph != null) {
				...
				ProfileManager.getInstance().setProperty(config.getEntityID(),
						config);
				updateEntityObservers();
			}
		...
		case RETURN_QUEST:
			...
			EntityConfig configReturnProperty = ProfileManager
					.getInstance()
					.getProperty(configReturn.getEntityID(), EntityConfig.class);
			if (configReturnProperty == null)
				return;
			...
			if (_questUI.isQuestReadyForReturn(questID)) {
				...
				configReturnProperty
						.setConversationConfigPath(QuestUI.FINISHED_QUEST);
				ProfileManager.getInstance().setProperty(
						configReturnProperty.getEntityID(),
						configReturnProperty);
			}
		...
		case ADD_ENTITY_TO_INVENTORY:
			...
			if (_inventoryUI.doesInventoryHaveSpace()) {
				_inventoryUI.addEntityToInventory(entity, entity
						.getEntityConfig().getCurrentQuestID());
				_mapMgr.clearCurrentSelectedMapEntity();
				_conversationUI.setVisible(false);
	...
	public void onNotify(String value, StoreInventoryEvent event) {
		...
		case PLAYER_INVENTORY_UPDATED:
			Array<InventoryItemLocation> items = _json.fromJson(Array.class,
					value);
			InventoryUI.populateInventory(_inventoryUI.getInventorySlotTable(),
					items, _inventoryUI.getDragAndDrop(),
					InventoryUI.PLAYER_INVENTORY, false);
			break;
		default:
			break;
	...
	public void onNotify(int value, StatusEvent event) {
		...
		case UPDATED_GP:
			...
			ProfileManager.getInstance().setProperty("currentPlayerGP",
					_statusUI.getGoldValue());
			break;
		case UPDATED_HP:
			ProfileManager.getInstance().setProperty("currentPlayerHP",
					_statusUI.getHPValue());
			break;
		case UPDATED_LEVEL:
			ProfileManager.getInstance().setProperty("currentPlayerLevel",
					_statusUI.getLevelValue());
			break;
		case UPDATED_MP:
			ProfileManager.getInstance().setProperty("currentPlayerMP",
					_statusUI.getMPValue());
			break;
		case UPDATED_XP:
			ProfileManager.getInstance().setProperty("currentPlayerXP",
					_statusUI.getXPValue());
			break;
		case LEVELED_UP:
			notify(AudioObserver.AudioCommand.MUSIC_PLAY_ONCE,
	...
	public void render(float delta) {
		...
		_statusUI.setScaleX(_stage.getWidth() / 1280);
		_statusUI.setScaleY(_stage.getHeight() / 720);
		...
		_stage2.getBatch().begin();

		_stage2.getBatch().draw(dpad, 0, 0, _stage.getWidth() / 6,
				_stage.getWidth() / 6);

		int buttonSize = (int) (_stage.getWidth() / 10);
		int statPointButtons = (int) (_stage.getWidth() - 2 * buttonSize);

		_stage2.getBatch().draw(aButton, statPointButtons, 0, buttonSize,
				buttonSize);
		_stage2.getBatch().draw(bButton, statPointButtons + buttonSize, 0,
				buttonSize, buttonSize);
		_stage2.getBatch().draw(xButton, statPointButtons, buttonSize,
				buttonSize, buttonSize);
		_stage2.getBatch().draw(yButton, statPointButtons + buttonSize,
				buttonSize, buttonSize, buttonSize);
		_stage2.getBatch().draw(lButton, 0,
				_stage.getHeight() - buttonSize / 2, 2 * buttonSize,
				buttonSize / 2);
		_stage2.getBatch().draw(rButton, statPointButtons,
				_stage.getHeight() - buttonSize / 2, 2 * buttonSize,
				buttonSize / 2);
		_stage2.getBatch().draw(startButton,
				statPointButtons / 2 - _stage.getWidth() / 12, 0,
				_stage.getWidth() / 6, _stage.getWidth() / 24);
		_stage2.getBatch().draw(selectButton,
				statPointButtons / 2 + _stage.getWidth() / 12, 0,
				_stage.getWidth() / 6, _stage.getWidth() / 24);

		_stage2.getBatch().end();
	...
	public void onNotify(Entity enemyEntity, BattleEvent event) {
		...
		case OPPONENT_DEFEATED:
			...
			int goldReward = Integer.parseInt(enemyEntity.getEntityConfig()
					.getPropertyValue(
							EntityConfig.EntityProperties.ENTITY_GP_REWARD
									.toString()));
			_statusUI.addGoldValue(goldReward);
			int xpReward = Integer.parseInt(enemyEntity.getEntityConfig()
					.getPropertyValue(
							EntityConfig.EntityProperties.ENTITY_XP_REWARD
									.toString()));
			_statusUI.addXPValue(xpReward);
			notify(AudioObserver.AudioCommand.MUSIC_STOP,
					AudioObserver.AudioTypeEvent.MUSIC_BATTLE);
		...
		case PLAYER_USED_MAGIC:
			notify(AudioObserver.AudioCommand.SOUND_PLAY_ONCE,
					AudioObserver.AudioTypeEvent.SOUND_PLAYER_WAND_ATTACK);
			int mpVal = ProfileManager.getInstance().getProperty(
					"currentPlayerMP", Integer.class);
			_statusUI.setMPValue(mpVal);
			break;
		default:
			break;
	...
	@Override
	public void notify(AudioObserver.AudioCommand command,
			AudioObserver.AudioTypeEvent event) {
