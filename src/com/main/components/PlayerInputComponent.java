...
public class PlayerInputComponent extends InputComponent {
	...
	private final static String TAG = PlayerInputComponent.class
			.getSimpleName();
	...
	public void receiveMessage(String message) {
		...
		if (string.length == 2) {
			if (string[0]
					.equalsIgnoreCase(MESSAGE.CURRENT_DIRECTION.toString())) {
				_currentDirection = _json.fromJson(Entity.Direction.class,
						string[1]);
			}
		}
	}
	...
	public void update(Entity entity, float delta) {
		...
		if (mouseButtons.get(Mouse.SELECT)) {
			// Gdx.app.debug(TAG, "Mouse LEFT click at : (" +
			// _lastMouseCoordinates.x + "," + _lastMouseCoordinates.y + ")" );
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		int startPoint_X = 0;
		int startPoint_Y = Gdx.graphics.getHeight();
		int padSize = Gdx.graphics.getWidth()/6;
		int arrowSize = padSize / 2 - padSize / 6;
		
		if (Gdx.input.getX() > startPoint_X + arrowSize 
				&& Gdx.input.getX() < startPoint_X + (padSize - arrowSize)
				&& Gdx.input.getY() < startPoint_Y - padSize + arrowSize
				&& Gdx.input.getY() > startPoint_Y - padSize + 0
				&& Gdx.input.isTouched()) {
		...
		if (Gdx.input.getX() > startPoint_X + arrowSize 
				&& Gdx.input.getX() < startPoint_X + (padSize - arrowSize)
				&& Gdx.input.getY() < startPoint_Y - padSize + padSize
				&& Gdx.input.getY() > startPoint_Y - padSize + (padSize - arrowSize)
				&& Gdx.input.isTouched()) {
		...
		if (Gdx.input.getX() > startPoint_X 
				&& Gdx.input.getX() < startPoint_X + arrowSize
				&& Gdx.input.getY() < startPoint_Y - padSize + (padSize - arrowSize)
				&& Gdx.input.getY() > startPoint_Y - padSize + arrowSize
				&& Gdx.input.isTouched()) {
		...
		if (Gdx.input.getX() > startPoint_X + (padSize - arrowSize) 
				&& Gdx.input.getX() < startPoint_X + padSize
				&& Gdx.input.getY() < startPoint_Y - padSize + (padSize - arrowSize)
				&& Gdx.input.getY() > startPoint_Y - padSize + arrowSize
				&& Gdx.input.isTouched()) {
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (Gdx.input.getX() > startPoint_X + arrowSize
				&& Gdx.input.getX() < startPoint_X + (padSize - arrowSize)
				&& Gdx.input.getY() < startPoint_Y - padSize + arrowSize
				&& Gdx.input.getY() > startPoint_Y - padSize 
				&& Gdx.input.isTouched()) {
			keys.put(Keys.UP, true);
		} else {
			keys.put(Keys.UP, false);
